...
같은 시드로 같은 상태를 만든 뒤, **고정 신호**와 **Solar Pro 4 가 세운 계획**을 각각 30초씩 돌린 영상입니다. 두 실행의 차이는 신호 시간뿐입니다.

| Before AI (고정 신호) | After AI (12초 / 8초 / 0초 적용) |
|----------|---------|
| <img src="docs/flowlight_before_ai.gif" width="100%"> | <img src="docs/flowlight_after_ai.gif" width="100%"> |

| 지표 (30초 후) | 고정 신호 | FlowLight AI | 변화 |
|:--------|-------------:|-------------:|------------:|
| 🚗 정지 차량 | **34대** | **21대** | **⬇ 38.2%** |
| 🚦 통과 차량 (최근 1분) | **106대** | **123대** | **⬆ 16.0%** |
| 📈 정체 지수 | **0.93** | **0.73** | **⬇ 21.1%** |

- 조건: 수동 모드 4x4 격자, 교통량 5대/초, 시드 20260702, 워밍업 120초 시점에 차량 49대·정지 33대·정체 지수 0.94.
- 이 상태를 실제 Solar Pro 4 에 보낸 결과: 계획 남북 12초 / 동서 8초 / 보행자 0초, Guardrail 보정 없음, 평가 88점, 최종 판단 `자동 적용`.
- 30초 평균은 정지 차량 33.4대 → 28.6대, 정체 지수 0.915 → 0.857 입니다. 신호 주기에 따라 순간값이 오르내리므로 평균도 같이 적었습니다.
...
## 🚶 보행 신호는 어떻게 적용되나
계획의 보행 녹색 N초는 시뮬레이터에서 **보행 전용 현시**가 됩니다. 남북 차량 녹색 → 동서 차량 녹색 → 모든 차량 정지 + 모든 횡단보도 녹색 N초 순서로 한 주기가 돕니다.
N이 0이면 현시 자체가 없어서 주기 전체가 차량에 갑니다. 그래서 "보행자가 없으면 보행 신호를 건너뛴다"와 "교통약자가 있으면 보행 시간을 늘린다"가 숫자가 아니라 화면에서 그대로 보입니다.
AI 적용 전의 기본 신호는 예전처럼 교차 방향 차량 녹색과 나란히 보행 녹색이 켜집니다.
...
- AI 계획의 보행 녹색은 보행 전용 현시로 적용(0초면 생략). 적용 후 효과는 시뮬레이션 시간 15초 평균으로 측정하므로 배속을 걸면 그만큼 빨리 나옴
...
| 수동 모드 4x4, 차량 49대·정지 33대 (전후 비교 GIF 의 분기 상태) | 계획 12/8/0, Guardrail 보정 없음, 88점, 자동 적용 → 30초 후 정지 34 → 21대 |
...
횡단보도에 노약자가 서 있는 상태로 분석한 실제 실행입니다. Agent 1 이 교통약자를 감지했고, Agent 2 는 "교통약자 1명이 있어 횡단 속도를 고려해 10초" 라는 근거로 12/8/10 을 냈습니다. 적용 후 모든 차가 멈추고 모든 횡단보도가 10초 녹색이 되는 현시가 생겼습니다. 같은 자리에 보행자가 없으면 이 현시는 0초가 되어 사라집니다.
...
- 적용 후 실제 주기는 차량 녹색 합 + 보행 현시 + 황색·전적색(방향당 3초, 보행 현시 뒤 1초)이라 입력 주기(`cycle_sec`)보다 깁니다. 보행 녹색 0초가 적용된 뒤에 새로 온 보행자는 다음 AI 분석 때까지 기다립니다.
- AI 계획은 좌회전 전용 현시를 주지 않아 적용 후에는 직진 신호에 비보호 좌회전이 됩니다.
