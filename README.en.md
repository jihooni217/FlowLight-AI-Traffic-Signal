...
- The plans are applied the way a real signal system would: each direction's green is split 70 % through / 30 % left to keep the four-phase cycle (with two or more lanes the left turn is protected, from a dedicated lane), and every cycle the left-turn and pedestrian phases are added or skipped from actual demand. The method matches the built-in A/B check under Advanced settings (same seed, fixed 1/30 s step, each run started from the same warmed-up state).
...
|---|---|---|
| `UPSTAGE_API_KEY` | (needed for live analysis) | Issued in the Upstage console. Without it the server still starts and AI analysis falls back to the recorded replay |
...
### Trying it without an API key (recorded replay)

You can see the whole flow without an API key. **AI analysis** then replays a **real Solar Pro 4 response** stored in `app/replay_data.js`, with the original timing between events.
